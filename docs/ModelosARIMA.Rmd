...
Dado que, $E[\varepsilon_t x_{t-j}]=0$   
...
resolvemos entonces para $\gamma_0$, $\gamma_1$ y generalizamos para $\gamma_j$. Es fácil ver que $\gamma_0$ es $Var(x)t)$
...
\visible<2->{  & = E[\varepsilon_t^2] - 2\theta E[\varepsilon_t \varepsilon_{t-1}] + \theta^2 E[\varepsilon_{t-1}^2]} \\
\visible<3->{    & = \sigma^2 + \theta^2 \sigma^2 \\
                & = \sigma^2 (1+\theta^2)}
\end{align*}
...
\visible<2->{     & = E[\varepsilon_t \varepsilon_{t-j}] - \theta E[ \varepsilon_{t-1} \varepsilon_{t-j}] 
                        - \theta E[ \varepsilon_{t} \varepsilon_{t-j-1}] + \theta^2 E[ \varepsilon_{t-1} \varepsilon_{t-j-1}]}
\end{align*}    
...
\begin{align*}
    \gamma_1     & = E[\varepsilon_t \varepsilon_{t-1}] - \theta E[ \varepsilon_{t-1} \varepsilon_{t-1}] 
                        - \theta E[ \varepsilon_{t} \varepsilon_{t-2}] + \theta^2 E[ \varepsilon_{t-1} \varepsilon_{t-2}] \\
\visible<2->{                 & =  0  - \theta \sigma^2 - 0 + 0 \\
                & =   - \theta \sigma^2 }      
\end{align*}    

\visible<3->{si $j=2$,
\begin{align*}
    \gamma_2     & = E[\varepsilon_t \varepsilon_{t-2}] - \theta E[ \varepsilon_{t-1} \varepsilon_{t-2}] 
                        - \theta E[ \varepsilon_{t} \varepsilon_{t-3}] + \theta^2 E[ \varepsilon_{t-1} \varepsilon_{t-3}] \\
                 & = 0
\end{align*} 
}
...
\begin{align*}
    \gamma_j     & =  E[\varepsilon_t \varepsilon_{t-j}] - \theta E[ \varepsilon_{t-1} \varepsilon_{t-j}] 
                        - \theta E[ \varepsilon_{t} \varepsilon_{t-j-1}] + \theta^2 E[ \varepsilon_{t-1} \varepsilon_{t-j-1}] \\
                 & = 0
\end{align*} 

\visible<2->{Noten que para cualquier valor de $\theta$, el proceso es estacionario débil, ya que no se deben imponer restricciones sobre el parámetro para obtener la media, varianza o auto-covarianzas }
...
\visible<2->{     & = \frac{- \theta }{ (1+\theta^2)} }
\end{align*}

\visible<3->{y dado que $\gamma_j=0$ para todo $j>1$, esto implica que $\rho_j=0$ para todo $j>1$}
...
\visible<2->{  & = x_t + \theta x_{t-1} + \theta^2 (x_{t-2} + \theta \varepsilon_{t-3}) } \\
\visible<3->{  &     \vdots \\
...
Si $| \theta | < 1$ el termino $\theta^n \varepsilon_{t-n} \rightarrow 0$ cunado $n \rightarrow \infty$, por lo cual bajo esta condición podemos expresar el proceso como

\begin{align*}
  \varepsilon_t  & = x_t + \theta x_{t-1} + \theta^2 x_{t-2} + \dots + \theta^{n-1} x_{t-n-1} 
\end{align*}

\visible<2->{De esta forma pasamos de un modelo MA(1) a un AR($\infty$), a la condición $|\theta| < 1$ la llamamos la condición de invertibilidad}
...
\begin{equation}\label{inv:ma1}
    \varepsilon_ = \frac{1}{1-\theta L} x_t
\end{equation}

Como se ve en la ecuación \ref{inv:ma1} la condición de invertibilidad de un MA(1) es equivalente a la condición de estacionariedad de un modelo AR(1). \visible<2->{Cabe recalcar de nuevo que el proceso MA(1) siempre es estacionario!, y la condición de invertibilidad es solo para poder pasar a un modelo AR($\infty$)}
\end{frame}
...
\visible<2->{  &  \text{Dado que la esperanza de todos los términos cruzados es cero,} } \\
\visible<3->{    & = \sigma^2 + \theta_1^2 \sigma^2 + \theta_2^2 \sigma^2 + \dots + \theta_q^2 \sigma^2 \\
...
\visible<2->{Sabemos que la esperanza de los términos $\varepsilon$ que no estén en el mismo periodo es 0. Entonces, en este caso obtendríamos,
...
\visible<2->{Entonces, en este caso obtendríamos,
...
Como hicimos para los modelos AR y MA, comenzamos con el modelo más sencillo y construimos desde ahí.

\visible<2->{Definimos el proceso ARMA(1,1) como,
...
\begin{align*}
\gamma_j & = E[x_t x_{t-j}] \\
         & = \phi E[x_{t-1} x_{t-j}] + E[\varepsilon_t  x_{t-j}] - \theta [\varepsilon_{t-1} x_{t-j}] \\
         & = \phi \gamma_{j-1} + E[\varepsilon_t  x_{t-j}] - \theta [\varepsilon_{t-1} x_{t-j}]
\end{align*}
...
\begin{align*}
\gamma_1 & = \phi \gamma_0 + E[\varepsilon_t  x_{t-1}]  + E[\varepsilon_t  x_{t-j}] - \theta [\varepsilon_{t-1} x_{t-1}] \\
         & = \phi \frac{(1+\theta^2- 2 \theta \phi) \sigma^2}{1-\phi^2} - \theta \sigma^2
\end{align*}

\visible<2->{Y para $j>1$ obtenemos
\begin{align*}
\gamma_j & = \phi \gamma_{j-1} + E[\varepsilon_t  x_{t-j}]  + E[\varepsilon_t  x_{t-j}] - \theta [\varepsilon_{t-1} x_{t-j}] \\
         & = \phi \gamma_{j-1}
\end{align*}
...
\visible<2->{         & = \phi_1 \gamma_{j-1} + \dots + \phi_p \gamma_{j-p} + E[\varepsilon_t  x_{t-j}] \\
         & - \theta_1 E[\varepsilon_{t-1} x_{t-j}] - \dots - \theta_q E[\varepsilon_{t-q} x_{t-j}] }
\end{align*}
...
\visible<2->{Y para $j>q$ obtenemos
\begin{align*}
\gamma_j & = \phi_1 \gamma_{j-1} + \dots + \phi_p \gamma_{j-p} 
\end{align*}
...
\visible<2->{Si existen dos raíces iguales, digamos $\lambda_1 = \delta_q$ podemos expresar el procesos como un ARMA(p-1,q-1),
...
\begin{frame}{ARIMA(p,d,q)}\label{arima}
...
\begin{frame}{Transformación Box-Cox}\label{transf}
